"use client";

import { useFormStatus } from "react-dom";
import type { RefObject } from "react";
import { cn } from "@/lib/cn";
import { useMagnetic, MAGNETIC_TRANSITION } from "@/lib/useMagnetic";
import { signInWithGoogle } from "@/app/actions";

type SignInButtonProps = {
  children: React.ReactNode;
  className?: string;
  /** Apply the magnetic-pull hover (for the big landing CTAs). */
  magnetic?: boolean;
  /** Show the Google "G" before the label. */
  withIcon?: boolean;
  strength?: number;
};

export function SignInButton({
  children,
  className,
  magnetic = false,
  withIcon = false,
  strength = 14,
}: SignInButtonProps) {
  // `contents` keeps the form layout-transparent so it doesn't disturb flex rows.
  return (
    <form action={signInWithGoogle} className="contents">
      <Submit className={className} magnetic={magnetic} withIcon={withIcon} strength={strength}>
        {children}
      </Submit>
    </form>
  );
}

function Submit({
  children,
  className,
  magnetic,
  withIcon,
  strength = 14,
}: Omit<SignInButtonProps, "children"> & { children: React.ReactNode }) {
  const { ref, onMouseMove, onMouseLeave } = useMagnetic(strength);
  const { pending } = useFormStatus();

  return (
    <button
      ref={magnetic ? (ref as RefObject<HTMLButtonElement>) : undefined}
      type="submit"
      disabled={pending}
      onMouseMove={magnetic ? onMouseMove : undefined}
      onMouseLeave={magnetic ? onMouseLeave : undefined}
      className={cn("disabled:opacity-70", className)}
      style={magnetic ? { transition: MAGNETIC_TRANSITION } : undefined}
    >
      {withIcon && <GoogleMark />}
      {pending ? "Redirecting…" : children}
    </button>
  );
}

function GoogleMark() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden="true">
      <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.27-4.74 3.27-8.1Z" />
      <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.65l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23Z" />
      <path fill="#FBBC05" d="M5.84 14.11a6.6 6.6 0 0 1 0-4.22V7.05H2.18a11 11 0 0 0 0 9.9l3.66-2.84Z" />
      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1A11 11 0 0 0 2.18 7.05l3.66 2.84C6.71 7.3 9.14 5.38 12 5.38Z" />
    </svg>
  );
}
